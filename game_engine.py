...
# Константы\STARTING_STACK = 1000
BLIND_SMALL    = 1
BLIND_BIG      = 2
MIN_PLAYERS    = 2  # минимальное число для старта новой раздачи


# Типы раундов: pre-flop, flop, turn, river, showdown
...
def start_hand(table_id: int):
    ...
    # Подготавливаем колоду и раздаём
    deck = new_deck()
...
def apply_action(table_id: int, uid: str, action: str, amount: int = 0):
    ...
    # 1) Fold — автоматическая победа оппонента
    if action == "fold":
        ...
        state["revealed_hands"] = {
            uid: state["hole_cards"][uid],
            opponent: state["hole_cards"][opponent]
        }
        state["started"] = False
        # Рестарт, если достаточно игроков
    ...
    # 4) Переход улицы при равных вкладах
    if all(contrib.get(p, 0) == state.get("current_bet", 0) for p in active):
        ...
        if rnd == "pre-flop":
            ...
            # Showdown: раскрываем руки
            players = state["players"]
            state["revealed_hands"] = {pid: state["hole_cards"][pid] for pid in players}
            # TODO: реализовать сравнитель комбинаций для установки победителя
